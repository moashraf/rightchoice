                ...
                <div class="rc-buyer-packages__grid">
                    @foreach($packages as $package)
                        ...
                        <article class="rc-buyer-plan" style="--delay: {{ $loop->index * 120 }}ms">
                            ...
                            <button
                                type="button"
                                class="rc-buyer-plan__details-button"
                                data-rc-package-modal-open="rc-package-modal-{{ $package->id }}"
                                aria-haspopup="dialog"
                            >
                                <i class="fas fa-info-circle"></i>
                                <span>{{ $isEnglish ? 'Package details' : 'تفاصيل الباقة' }}</span>
                            </button>
                        ...
                        <div
                            id="rc-package-modal-{{ $package->id }}"
                            class="rc-package-modal"
                            role="dialog"
                            aria-modal="true"
                            aria-labelledby="rc-package-modal-title-{{ $package->id }}"
                            dir="rtl"
                            hidden
                        >
                            <div class="rc-package-modal__panel" role="document">
                                <button
                                    type="button"
                                    class="rc-package-modal__close"
                                    data-rc-package-modal-close
                                    aria-label="إغلاق"
                                >
                                    <i class="fas fa-times"></i>
                                </button>

                                <div class="rc-package-modal__icon">
                                    <i class="fas fa-info-circle"></i>
                                </div>

                                <span class="rc-package-modal__eyebrow">تفاصيل الباقة</span>
                                <h3 id="rc-package-modal-title-{{ $package->id }}">{!! $packageName !!}</h3>

                                @if(!empty($packageDescription))
                                    <p class="rc-package-modal__description">{{ strip_tags($packageDescription) }}</p>
                                @endif

                                @if(!empty($packageDetails))
                                    <ul class="rc-package-modal__details">
                                        @foreach($packageDetails as $detail)
                                            <li>
                                                <i class="fas fa-check"></i>
                                                <span>{{ strip_tags($detail) }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                                    <p class="rc-package-modal__empty">لا توجد تفاصيل إضافية لهذه الباقة.</p>
                                @endif

                                <button type="button" class="rc-package-modal__done" data-rc-package-modal-close>
                                    إغلاق
                                </button>
                            </div>
                        </div>
...
.rc-buyer-packages{background:linear-gradient(145deg,#052c55,#0b5cab)}.rc-buyer-plan{position:relative;padding:30px;border:1px solid #ffffff30;border-radius:26px;background:#fff;box-shadow:0 22px 55px #00172d52;animation:rcBuyerPlanEnter .65s ease backwards;animation-delay:var(--delay);transition:.3s}.rc-buyer-plan:hover{transform:translateY(-10px);box-shadow:0 30px 65px #00172d80}.rc-buyer-plan__badge{position:absolute;top:18px;left:18px;padding:6px 10px;border-radius:999px;background:#0b5cab;color:#fff;font-weight:950}.rc-buyer-plan h3{padding-inline-end:65px;font-size:22px;font-weight:950}.rc-buyer-plan__description{min-height:48px;color:#66798c;line-height:1.8}.rc-buyer-plan__price{display:grid;gap:12px;margin:22px 0;padding:18px;border-radius:18px;background:#edf7ff}.rc-buyer-plan__price-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.rc-buyer-plan__price-row>span{font-size:13px;font-weight:900;color:#60778b}.rc-buyer-plan__price-row--before{padding-bottom:11px;border-bottom:1px dashed #bad6eb}.rc-buyer-plan__price del{color:#8798a8;font-size:16px;font-weight:800}.rc-buyer-plan__price strong{color:var(--blue);font-size:30px;font-weight:950}.rc-buyer-plan__price small{font-size:14px}.rc-buyer-plan__details-button{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;margin:0 0 17px;padding:11px 14px;border:1px solid #a8d1f1;border-radius:12px;background:#edf7ff;color:var(--blue);font-family:inherit;font-weight:950;cursor:pointer;transition:.25s}.rc-buyer-plan__details-button:hover,.rc-buyer-plan__details-button:focus{border-color:var(--blue);background:#dcefff;transform:translateY(-2px);outline:none}.rc-buyer-plan ul{     text-align: right; padding:0;list-style:none;line-height:2.1}.rc-buyer-plan li i{margin-inline-end:8px;color:#159447}.rc-buyer-plan__features{margin-bottom:20px}.rc-buyer-plan>a{display:flex;justify-content:center;gap:10px;padding:14px;border-radius:13px;background:linear-gradient(135deg,var(--blue),var(--cyan));color:#fff!important;font-weight:900;text-decoration:none!important}
...
.rc-package-modal[hidden]{display:none!important}.rc-package-modal{position:fixed;inset:0;z-index:10050;display:grid;place-items:center;padding:20px;background:rgba(2,23,43,.72);backdrop-filter:blur(6px);animation:rcPackageModalFade .22s ease both}.rc-package-modal__panel{position:relative;width:min(560px,100%);max-height:85vh;overflow:auto;padding:34px 30px 28px;border:1px solid #cfe3f4;border-radius:26px;background:#fff;box-shadow:0 30px 80px rgba(0,24,48,.35);direction:rtl;text-align:right;animation:rcPackageModalEnter .3s cubic-bezier(.22,1,.36,1) both}.rc-package-modal__close{position:absolute;top:16px;left:16px;display:grid;place-items:center;width:38px;height:38px;border:0;border-radius:50%;background:#edf5fb;color:#36566f;cursor:pointer}.rc-package-modal__icon{display:grid;place-items:center;width:58px;height:58px;margin-bottom:16px;border-radius:18px;background:#e5f3ff;color:var(--blue);font-size:23px}.rc-package-modal__eyebrow{color:var(--blue);font-size:13px;font-weight:950}.rc-package-modal h3{margin:7px 0 12px;padding:0;color:var(--dark);font-size:25px;font-weight:950;text-align:right}.rc-package-modal__description{margin:0 0 18px;color:#60778b;line-height:1.9;text-align:right}.rc-package-modal__details{display:grid;gap:10px;margin:0;padding:0;list-style:none}.rc-package-modal__details li{display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1px solid #dceaf5;border-radius:13px;background:#f8fbfe;color:#354f65;line-height:1.8;text-align:right}.rc-package-modal__details li i{flex:0 0 auto;margin-top:6px;color:#159447}.rc-package-modal__empty{padding:15px;border-radius:13px;background:#f6f8fa;color:#718294;text-align:right}.rc-package-modal__done{display:block;width:100%;margin-top:22px;padding:13px;border:0;border-radius:13px;background:linear-gradient(135deg,var(--blue),var(--cyan));color:#fff;font-family:inherit;font-weight:950;cursor:pointer}@keyframes rcPackageModalFade{from{opacity:0}to{opacity:1}}@keyframes rcPackageModalEnter{from{opacity:0;transform:translateY(25px) scale(.96)}to{opacity:1;transform:none}}
@media(prefers-reduced-motion:reduce){.rc-buyer-promo *,.rc-package-modal,.rc-package-modal__panel{animation:none!important;transition:none!important}}
</style>
...
(function () {
    ...
    function openModal(modal) {
        ...
        modal.hidden = false;
        var closeButton = modal.querySelector('[data-rc-package-modal-close]');
    ...
    document.addEventListener('click', function (event) {
        var openButton = event.target.closest('[data-rc-package-modal-open]');
        if (openButton) {
            openModal(document.getElementById(openButton.getAttribute('data-rc-package-modal-open')));
            return;
        }

        var closeButton = event.target.closest('[data-rc-package-modal-close]');
        if (closeButton) {
            closeModal(closeButton.closest('.rc-package-modal'));
            return;
        }

        if (event.target.classList.contains('rc-package-modal')) {
