{{-- WhatsApp Batch Details Page --}}
@extends('layouts.admin')

@section('title', 'تفاصيل دفعة واتساب #' . $batch->id)
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><i class="fab fa-whatsapp"></i> تفاصيل الدفعة #{{ $batch->id }}</h1>
                </div>
                <div class="col-sm-6">
    ...
    <div class="content px-3">
        ...
        {{-- Retry Failed Button --}}
        @if($stats['failed'] > 0 && auth()->guard('admin')->user()->hasPermission('whatsapp.send'))
            <div class="mb-3">
                <form action="{{ route('sitemanagement.whatsapp.retry', $batch->id) }}" method="POST" class="d-inline"
                      onsubmit="return confirm('هل أنت متأكد من إعادة إرسال رسائل واتساب الفاشلة؟')">
                    @csrf
        ...
        @if($stats['pending'] > 0)
            <div class="alert alert-info">
                ...
                يوجد <strong>{{ $stats['pending'] }}</strong> رسالة واتساب قيد المعالجة. يمكنك تحديث الصفحة لمتابعة التقدم.
                <a href="{{ route('sitemanagement.whatsapp.show', $batch->id) }}" class="btn btn-sm btn-info mr-2">
