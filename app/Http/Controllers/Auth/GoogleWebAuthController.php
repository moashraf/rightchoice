...
class GoogleWebAuthController extends Controller
{
    public function login(Request $request, SocialTokenVerifier $verifier, ?string $locale = null): RedirectResponse
    {
        ...
        try {
            $claims = $verifier->verify('google', $data['credential']);
        } catch (UnexpectedValueException | InvalidArgumentException | DomainException $e) {
            return back()->withErrors(['google' => 'تعذر التحقق من حساب Google. حاول مرة أخرى.']);
        } catch (ConnectionException | RequestException | GuzzleException $e) {
            return back()->withErrors(['google' => 'خدمة Google غير متاحة مؤقتًا. حاول مرة أخرى.']);
        }
        ...
        try {
            $user = DB::transaction(function () use ($claims) {
                ...
                $name = $claims['name'] ?? 'RightChoice user';
            ...
            $message = match ($e->getMessage()) {
                ...
                default => 'يجب أن يوفر حساب Google بريدًا إلكترونيًا موثقًا.',
            };

            return back()->withErrors(['google' => $message]);
        } catch (QueryException $e) {
            if (in_array($e->errorInfo[1] ?? null, [1062, 19], true) || $e->getCode() === '23505') {
                return back()->withErrors(['google' => 'حدث تعارض أثناء إنشاء الحساب. حاول تسجيل الدخول مرة أخرى.']);
            }
